...
public class ActivitiHistoryGraphBuilder {
	...
	public void fetchProcessDefinitionEntity() {
		String processDefinitionId = Context.getCommandContext().getHistoricProcessInstanceEntityManager()
				.findHistoricProcessInstance(this.processInstanceId).getProcessDefinitionId();
	...
	public Edge findPreviousEdge(Node currentNode, long currentStartTime) {
		...
		for (PvmTransition pvmTransition : activityImpl.getIncomingTransitions()) {
			...
			HistoricActivityInstance visitiedHistoryActivityInstance = findVisitedHistoricActivityInstance(
					previousActivityId);

			if ((visitiedHistoryActivityInstance == null) || (visitiedHistoryActivityInstance.getEndTime() == null)) {
				continue;
			}
			...
			} else if (currentStartTime - nestestHistoricActivityInstance.getEndTime().getTime() > currentStartTime
	...
	public HistoricActivityInstance findVisitedHistoricActivityInstance(String activityId) {
		for (int i = this.visitedHistoricActivityInstances.size() - 1; i >= 0; i--) {
			HistoricActivityInstance historicActivityInstance = (HistoricActivityInstance) this.visitedHistoricActivityInstances
					.get(i);
